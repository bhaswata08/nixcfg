...
- No em dashes and no en dashes. STE itself does not ban them, but this user does. Use a period, a comma, or a rewrite.
...
For a machine check, run the bundled linter on the draft file:

```
python3 ~/.claude/skills/ste-writing/ste-lint.py your-draft.md
```

This machine is NixOS and has no `python3` on the PATH. Use this form instead:

```
nix-shell -p python3 --run "python3 ~/.claude/skills/ste-writing/ste-lint.py your-draft.md"
```

It reports violations per 100 words. Lower is cleaner. Lint before and after the rewrite. The drop between the two scores is the signal.
